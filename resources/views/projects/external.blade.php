    ...
    <div class="bg-eids-primary/5 border border-eids-primary/15 rounded-2xl p-5 mb-6 flex items-start gap-3.5 shadow-2xs">
        ...
        <div>
            ...
            <div class="text-xs text-gray-700 mt-1 leading-relaxed font-medium">
                Mark which external elements exist on this project. Turning one on generates its sample units (Table 6);
                the External Works score is a flat pass-rate across every present element's checklist answers.
            </div>
        </div>
    </div>
                    ...
                    <div class="px-6 py-4">
                        <div class="flex items-center justify-between gap-4 flex-wrap">
                            <div>
                                ...
                                <div class="text-xs text-gray-500 mt-0.5">{{ $el->sample_count }} sample unit(s) per Table 6</div>
                            </div>
                            <form method="POST" action="{{ route('projects.external.toggle', [$project, $el->element_code]) }}">
                                @csrf
                                <input type="hidden" name="present" value="{{ $present ? '0' : '1' }}">
                                <button type="submit"
                                        class="min-h-[38px] px-4 py-2 rounded-xl text-xs font-extrabold transition {{ $present ? 'bg-emerald-100 text-emerald-800 border border-emerald-300 hover:bg-emerald-200' : 'bg-gray-100 text-gray-600 border border-gray-200 hover:bg-gray-200' }}">
                                    {{ $present ? 'Present — click to remove' : 'Not present — click to add' }}
                                </button>
                            </form>
                        </div>
